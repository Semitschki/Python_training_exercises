...
def _create_csv_header(path, header):
...
def _get_csv_data(path):
    ...
    with open(path, 'r', newline='', encoding='ISO-8859-1') as csv_file:
        ...
        try:
            ...
            empty_csv = _create_csv_header(path, CSV_HEADER)
            return empty_csv
...
def _delete_csv_data(path, music):
    '''Function to delete music.'''
    all_music = _get_csv_data(path)
    with open(path, 'w+', newline='', encoding='ISO-8859-1') as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        for idx, line in enumerate(all_music):
            if ((music[ALBUM] != line[ALBUM].lower()) or
                    (music[ARTIST].lower() != line[ARTIST].lower()) or
                    (music[TRACK].lower() != line[TRACK].lower())):
                continue
            del all_music[idx]
        all_music.insert(0, CSV_HEADER)
...
def _patch_csv_data(path, music, music_change):
    '''Function to patch data in a CSV.'''
    if len(music_change) == 1:
        music_change.extend(music[ALBUM], music[TRACK])
    if len(music_change) == 2:
        music_change.append(music[TRACK])
    all_music = _get_csv_data(path)
    if not all_music:
        return False
    with open(path, 'w', newline='', encoding='ISO-8859-1') as csv_file:
        ...
        for idx, line in enumerate(all_music):
            if ((music[ALBUM] != line[ALBUM].lower()) or
                    ...
                    (music[TRACK].lower() != line[TRACK].lower())):
                continue
            all_music[idx] = music_change
        all_music.insert(0, CSV_HEADER)
        for item in all_music:
            writer.writerow(item)
        return sorted(all_music)
...
class HTTPServer:
    ...
    def _handle_request(self, request):
        ...
        if not os.path.isfile(FILE_PATH):
            _create_csv_header(FILE_PATH, CSV_HEADER)
        ...
        if request.resource.split('/')[1] == 'Musik':
            ...
            if request.method == 'DELETE':
                ...
                _delete_csv_data(FILE_PATH, music)
                return Response(
                    create_status_code('OK'),
                    reply_header,
                    create_status_code('OK', True)
            ...
            if request.method == 'PATCH':
                music = request.resource.lower().split('/')[2:5]
                try:
                    music_change = request.body.lower().split('/')[0:3]
                except (ValueError, AttributeError):
                    return Response(
                        create_status_code('Bad Request'),
                        reply_header,
                        '\nOnly valid format (artist/album/track):\n' +
                        '/Musik/string/string*/string*\n'
                    )
                patched = _patch_csv_data(FILE_PATH, music, music_change)
                if patched:
                    return Response(
                        create_status_code('OK'),
                        reply_header,
